...
public class PurchaseVerificationHandler extends BaseClientRequestHandler
{
	...
	private void sendResult(User sender, String productID, String purchaseToken)
	{
		...
		trace("Player Purchase --playerId:", game.player.id, "--market:", game.market, "--productID:", productID,
				"--purchaseToken:", purchaseToken, "--Hard Currency:",
				game.player.resources.get(ResourceType.R4_CURRENCY_HARD));
	...
	private void sendSuccessResult(User sender, Game game, String productID, String purchaseToken, int consumptionState,
			int purchaseState, String developerPayload, long purchaseTime) {
		ISFSObject resObj = SFSObject.newInstance();
		if (purchaseState != 0) {
			resObj.putBool("success", false);
	...
	HttpUtils.Data verify(String productID, String purchaseToken, String market) {
		...
		if (market.equals("myket"))
			...
			url = "https://pardakht.cafebazaar.ir/devapi/v2/api/validate/" + packageName + "/inapp/" + productID
					+ "/purchases/" + purchaseToken + "/?access_token=" + accessToken_cafebazaar;
